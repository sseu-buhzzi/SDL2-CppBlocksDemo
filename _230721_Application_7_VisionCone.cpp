...
void MainActivity::filter_imvisible_blocks() {
    ...
    struct BlockAndDepth{
        ::BlockInstance *block_inst;
        ::int32_t depth;
    };
    ::int64_t coord[3];
    // ::std::vector<BlockInstance *> temp_visible_block_list;
    ::std::deque<::std::unordered_set<::BlockInstance *>> temp_visible_block_list;
    ...
    //             if (block_displayed.insert(block_inst).second) {
    //                 temp_visible_block_list.emplace_back(block_inst);
    //             }
    //         }
    //     }
    // }
    ::std::vector<::std::pair<::double_t[3], BlockAndDepth>> light_vectors, deeper_light_vectors;
    // ::std::unordered_set<::double_t *> blocked_light_vectors;
    for (::double_t viewport_x{-1.0}, viewport_vector[3]; viewport_x <= 1.0; viewport_x += 0.01) {
        for (::double_t viewport_y{-1.0}; viewport_y <= 1.0; viewport_y += 0.01) {
            light_vectors.emplace_back();
            if (!this->camera.get_viewport_vector(viewport_x, viewport_y, light_vectors.back().first)) {
                light_vectors.pop_back();
            }
        }
    }
    while (!light_vectors.empty()) {
        ::std::unordered_set<::double_t *> blocked_light_vectors;
        temp_visible_block_list.emplace_front();
        while (blocked_light_vectors.size() < light_vectors.size()) {
            for (::std::vector<::std::pair<::double_t[3], BlockAndDepth>>::iterator light_vector{light_vectors.begin()}; light_vector != light_vectors.end(); ++light_vector) {
                if (blocked_light_vectors.find(light_vector->first) == blocked_light_vectors.end()) {
                    this->camera.v_get_pixel_coord3llv(light_vector->first, ++light_vector->second.depth, coord);
                    ::BlockInstance *block_inst{this->chunk_map.get_block_inst(coord[0], coord[1], coord[2])};
                    if (block_inst->block->interaction_level >= 0x01) {
                        if (block_inst->block->interaction_level < 0x04) {
                            deeper_light_vectors.emplace_back(*light_vector);
                        }
                        blocked_light_vectors.emplace(light_vector->first);
                        temp_visible_block_list.front().insert(block_inst);
                    }
                    if (light_vector->second.depth > 64) {
                        blocked_light_vectors.emplace(light_vector->first);
                    }
                }
            }
        }
        // TCO << light_vectors.size() << '\t';
        // light_vectors = ::std::move(deeper_light_vectors);
        // TCO << light_vectors.size() << '\t';
        light_vectors = ::std::move(deeper_light_vectors);
        // TCO << light_vectors.size() << '\n';
        // TCO << temp_visible_block_list.front().size() << '\n';
        // light_vectors.clear();
        // blocked_light_vectors.clear();
        // TCO << deeper_light_vectors.size() << '\t';
    }
    // ::std::cout << "\n\n";
                                                                // temp_visible_block_list.emplace_front();
                                                                // for (::uint32_t i{0U}; i < ::Chunk::SIZE; ++i) {
                                                                ...
                                                                //     }
                                                                // }
                                                                // for (::std::vector<::std::pair<::double_t[3], ::int32_t>>::iterator light_vector{light_vectors.begin()}; light_vector != light_vectors.end(); ++light_vector) {
                                                                //     for (light_vector->second = 64; light_vector->second > 0 && this->camera.v_get_pixel_coord3llv(light_vector->first, light_vector->second, coord); --light_vector->second) {
                                                                //         BlockInstance *block_inst{this->chunk_map.get_block_inst(coord[0], coord[1], coord[2])};
                                                                //         if (block_inst->block->block_type != BT_AIR && block_displayed.insert(block_inst).second) {
                                                                //             temp_visible_block_list.front().emplace_back(block_inst);
                                                                //         }
                                                                //     }
                                                                // }
...
void MainActivity::world_display() {
    ...
    this->visible_block_list_mutex.lock();
    for (::std::deque<::std::unordered_set<::BlockInstance *>>::iterator visible_block_layer{this->visible_block_list.begin()}; visible_block_layer != this->visible_block_list.end(); ++visible_block_layer) {
                                                                ::std::cout << '\n' << visible_block_layer - this->visible_block_list.begin();
        for (::BlockInstance *visible_block : *visible_block_layer) {
            if (displayed_blocks.insert(visible_block).second) {
                visible_block->block->display(visible_block->coord[0], visible_block->coord[1], visible_block->coord[2]);
                                                                ::std::cout << '\t' << ::Sseu::BlockSource::block_name[visible_block->block];
            }
        }
    }
                                                                ::std::cout << "\n\n";
                                                                ...
                                                                ::glColor3ub(0xff, 0x00, 0x00);
                                                                for(::BlockInstance*visible_block:this->visible_block_list.front()){
                                                                    visible_block->block->select(visible_block->coord[0], visible_block->coord[1], visible_block->coord[2]);
                                                                }
                                                                ::glEnable(GL_TEXTURE_2D);
    this->visible_block_list_mutex.unlock();
    ::glDisable(GL_TEXTURE_2D);
