...
BlockInstance::BlockInstance(): not_refreshed{true}, rs_signal_strength{0}, linked_block{}, block{nullptr} { }
...
void CameraEntity::place_block() {
    ...
    if (placed_block->block->interaction_level == 0x00 && placed_block != &Trunk::null_block_inst) {
        ...
        Sseu::activity_stack.top<MainActivity>()->event_deque.emplace_back(GameEvent{GET_PLACE, this, placed_block});
    }
}
void CameraEntity::break_block() {
    Sseu::activity_stack.top<MainActivity>()->event_deque.emplace_back(GameEvent{GET_BREAK, this, this->camera->selected_block});
    ...
    if (event->block_inst != &Trunk::null_block_inst) {
        switch (event->type) {
            ...
            case GET_BREAK: {
                ...
                for (BlockInstance **linked_block{event->block_inst->linked_block}; linked_block - event->block_inst->linked_block < 6; ++linked_block) {
...
void MainActivity::refresh_block(BlockInstance *block_inst) {
    ...
    switch (block_inst->block->block_type) {
        case BT_RED_STONE_BLOCK: {
            block_inst->rs_signal_strength = 16;
            for (BlockInstance **linked_block{block_inst->linked_block}; linked_block - block_inst->linked_block < 6; ++linked_block) {
...
// std::this_thread::sleep_for(std::chrono::milliseconds(250));
            ...
            if (block_inst->linked_block != nullptr) {
                block_inst->linked_block[0]->rs_signal_strength = 1;
                this->refresh_block(block_inst->linked_block[0]);
            }
            // }
// TCO<<block_inst->coord[0]<<'\t'<<block_inst->rs_signal_strength<<'\n';
            break;
