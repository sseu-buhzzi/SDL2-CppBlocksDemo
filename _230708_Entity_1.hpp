...
// std::cout<<this->dz<<'\n';
        double_t box_top, new_x, new_y, new_z, rel_x, rel_y, rel_new_x, rel_new_y;
...
// TCO<<this->trunk_appended->get_block_info(rel_new_x - this->box_radius, rel_y, this->z)->block;
            ...
            double_t left_xmin{std::max(left_lower_edge, left_upper_edge) + int32_t(new_x - this->box_radius)};
            double_t right_xmax{std::min(right_lower_edge, right_upper_edge) + int32_t(new_x + this->box_radius)};
            double_t front_ymin{std::max(front_lower_edge, left_upper_edge) + int32_t(new_y - this->box_radius)};
            double_t back_ymax{std::min(back_lower_edge, back_upper_edge) + int32_t(new_y + this->box_radius)};
            double_t bottom_zmin{bottom_lower_edge + int32_t(new_z)};
            double_t top_zmax{top_upper_edge + int32_t(new_z + this->box_height)};
