...
void MainActivity::filter_imvisible_blocks() {
    ...
    //     }
    // }
    ::std::vector<::std::pair<::double_t[3], ::int32_t>> light_vectors;
    ::std::unordered_set<::double_t *> deeper_light_vectors;
    for (::double_t viewport_x{-1.0}, viewport_vector[3]; viewport_x <= 1.0; viewport_x += 0.01) {
        for (::double_t viewport_y{-1.0}; viewport_y <= 1.0; viewport_y += 0.01) {
    ...
    while (light_vectors.size() != deeper_light_vectors.size()) {
        for (::std::vector<::std::pair<::double_t[3], ::int32_t>>::iterator light_vector{light_vectors.begin()}; light_vector != light_vectors.end(); ++light_vector) {
            if (deeper_light_vectors.find(light_vector->first) == deeper_light_vectors.end()) {
                this->camera.v_get_pixel_coord3llv(light_vector->first, light_vector->second, coord);
                ::BlockInstance *block_inst{this->chunk_map.get_block_inst(coord[0], coord[1], coord[2])};
                if (block_inst->block->interaction_level >= 0x04) {
                    deeper_light_vectors.emplace(light_vector->first);
                    temp_visible_block_stack.top().insert(block_inst);
                }
                if (++light_vector->second > 64) {
                    deeper_light_vectors.emplace(light_vector->first);
                                                                ...
                                                                //             temp_visible_block_stack.top().emplace_back(block_inst);
                                                                //         }
                                                                //     }
                                                                // }
    this->visible_block_list_mutex.lock();
    this->visible_block_list.swap(temp_visible_block_stack);
    this->visible_block_list_mutex.unlock();
}
void MainActivity::world_display() {
    glEnable(GL_TEXTURE_2D);
    this->visible_block_list_mutex.lock();
    for (::std::stack<::std::unordered_set<::BlockInstance *>> temp_visible_block_list(this->visible_block_list); !temp_visible_block_list.empty(); temp_visible_block_list.pop()) {
        for (::BlockInstance *visible_block : temp_visible_block_list.top()) {
