...
void Camera::get_viewport_vector(double_t const viewport_x, double_t const viewport_y, double_t *viewport_vector) const {
    double_t const khi{viewport_x * M_PI * 0.375}, upsilon{viewport_y * M_PI_4};
    // double_t const khi{viewport_x * M_PI * 0.25}, upsilon{viewport_y * M_PI / 6};
    double_t const cos_kh{::cos(khi)};
    double_t const sin_u{::sin(upsilon)};
...
void Trunk::display_flush() const{
    for (uint32_t i{0U}; i < Trunk::SIZE; ++i) {
        for (uint32_t j{0U}; j < Trunk::SIZE; ++j) {
...
bool Trunk::contains_point_at(int32_t const x, int32_t const y, int32_t const z) const {
    return
        x >= 0 && y >= 0 && z >= 0
...
TrunkMap::TrunkMap(int64_t const front_offset, int64_t const back_offset, int64_t const left_offset, int64_t const right_offset): front_offset{front_offset}, back_offset{back_offset}, left_offset{left_offset}, right_offset{right_offset} {
...
BlockInstance *TrunkMap::get_block_inst(int64_t const x, int64_t const y, uint32_t z) const {
    if (z < 0 || z >= Trunk::SIZE) {
        return &::Sseu::BlockSource::null_block_inst;
    }
    ::int32_t const rel_x(::uint64_t(x) % ::Trunk::SIZE), rel_y(::uint64_t(y) % ::Trunk::SIZE);
    ::int32_t const x_index((x - rel_x) / ::Trunk::SIZE), y_index((y - rel_y) / ::Trunk::SIZE);
...
void TrunkMap::generate_trunk_at(int64_t const x_index, int64_t const y_index) {
...
void TrunkMap::link_trunk_at(int64_t const x_index, int64_t const y_index) {
    Trunk *trunk{this->origin[x_index] + y_index};
...
Entity::Entity(::TrunkMap *trunk_map, ::double_t const x, ::double_t const y, ::double_t const z, ::double_t const box_radius, ::double_t const box_height): trunk_map{trunk_map}, x{x}, y{y}, z{z}, dx{0.0}, dy{0.0}, dz{0.0}, box_radius{box_radius}, box_height{box_height}, is_existing{true} { }
Entity::~Entity() {
...
CameraEntity::CameraEntity(::TrunkMap *trunk_map, ::Camera *camera, ::double_t const x, ::double_t const y, ::double_t const z, ::double_t const theta, ::double_t const phi, ::double_t const box_radius, ::double_t const box_height): Entity(trunk_map, x, y, z, box_radius, box_height), camera{camera}, phi{phi}, theta{theta}, holding_block{::Sseu::BlockSource::block_enum[BBT_AIR]}, is_updating{true} { }
...
MainActivity::MainActivity():
    is_existing{true}, trunk_map(1LL, 2LL, 1LL, 2LL), camera_entity(&this->trunk_map, &this->camera, 2.5, 2.5, 16.0, 0.0, M_PI_2, 0.25, 1.75),
    screen_interface{SIT_GAME}, package_screen_interface(&this->camera_entity, {BT_TEST_BLOCK, BT_BLACK_BLOCK, BT_WHITE_BLOCK, BT_RED_STONE_POWDER, BT_RED_STONE_BLOCK}, {
        ::Sseu::BlockSource::test_block_btx.data[0],
        ::Sseu::BlockSource::black_block_btx.data[0],
        ::Sseu::BlockSource::white_block_btx.data[0],
        ::Sseu::BlockSource::red_stone_powder_btx.data[0],
...
void MainActivity::update() {
    ...
    if (!__TEST__::key_E_state && ::GetAsyncKeyState('E')) {
        if (this->screen_interface == SIT_GAME) {
            ::SDL_SetRelativeMouseMode(SDL_FALSE);
            this->camera_entity.is_updating = false;
            this->screen_interface = SIT_PACKAGE;
        } else {
            ::SDL_SetRelativeMouseMode(SDL_TRUE);
            this->camera_entity.is_updating = true;
            this->screen_interface = SIT_GAME;
        }
    }
...
void MainActivity::screen_display() {
    ...
    ::glPushMatrix();
                                                                static double_t font_size{18.0};
                                                                if(::GetAsyncKeyState(VK_UP)){font_size+=0.1;::std::cout<<font_size<<'\n';}
                                                                if(::GetAsyncKeyState(VK_DOWN)){font_size-=0.1;::std::cout<<font_size<<'\n';}
    std::string const *block_name{&::Sseu::BlockSource::block_name[this->camera_entity.holding_block]};
    // ::glTranslatef(-::Sseu::window_width * 0.5, -::Sseu::window_height * 0.5, 0.0F);
    // ::Sseu::FontSource::draw_text(" !\"#$%&'()*+,-./", font_size);
    // ::glTranslatef(0.0F, font_size * 2, 0.0F);
    // ::Sseu::FontSource::draw_text("0123456789", font_size);
    // ::glTranslatef(0.0F, font_size * 2, 0.0F);
    // ::Sseu::FontSource::draw_text(":;<=>?@", font_size);
    // ::glTranslatef(0.0F, font_size * 2, 0.0F);
    // ::Sseu::FontSource::draw_text("ABCDEFGHIJKLMNOPQRSTUVWXYZ", font_size);
    // ::glTranslatef(0.0F, font_size * 2, 0.0F);
    // ::Sseu::FontSource::draw_text("[\\]^_`", font_size);
    // ::glTranslatef(0.0F, font_size * 2, 0.0F);
    // ::Sseu::FontSource::draw_text("abcdefghijklmnopqrstuvwxyz", font_size);
    // ::glTranslatef(0.0F, font_size * 2, 0.0F);
    // ::Sseu::FontSource::draw_text("{|}~", font_size);
                                                                // static double_t offset{0.0};
                                                                // if(::GetAsyncKeyState(VK_LEFT)){offset+=0.1;::std::cout<<offset<<'\n';}
                                                                // if(::GetAsyncKeyState(VK_RIGHT)){offset-=0.1;::std::cout<<offset<<'\n';}
    ::glTranslatef(-(block_name->length() * font_size) * 0.5, -::Sseu::window_height * 0.5, 0.0F);
    ::Sseu::FontSource::draw_text(block_name->c_str(), font_size);
    ::glPopMatrix();
    ::glDisable(GL_TEXTURE_2D);
}
