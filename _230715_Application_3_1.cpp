...
void Camera::get_viewport_vector(double_t const viewport_x, double_t const viewport_y, double_t *viewport_vector) {
    double_t const khi{viewport_x * M_PI * 0.375}, upsilon{viewport_y * M_PI_4};
    // double_t const khi{viewport_x * M_PI * 0.25}, upsilon{viewport_y * M_PI / 6};
    double_t const cos_kh{cos(khi)};
    double_t const sin_u{sin(upsilon)};
...
bool Camera::get_pixel_coord3dv(double_t const depth, double_t *world_coord) {
...
bool Camera::get_pixel_coord3llv(double_t const depth, int64_t *world_coord) {
    if (std::isnan(*this->viewport_vector)) {
        return false;
    }
...
bool Camera::v_get_pixel_coord3llv(double_t *viewport_vector, double_t const depth, int64_t *world_coord) {
...
void Camera::select_block() {
    // this->viewport_vector_mutex.lock();
    this->set_viewport_vector(0.0, 0.0);
    // this->viewport_vector_mutex.unlock();
    for (double_t depth{0.0}, coord[3]; depth < 8.0; depth += 0.125) {
        if (this->get_pixel_coord3dv(depth, coord)) {
            ...
            if (this->selected_block->block->interaction_level >= 0x02) {
                ...
                // __TEST__::info(this->selected_block);
                break;
            }
        }
    }
...
void TrunkMap::generate_trunk_at(int64_t x_index, int64_t y_index) {
    Trunk *trunk{this->origin[x_index] + y_index};
    trunk->block_map = new BlockInstance **[Trunk::SIZE];
    for (uint32_t i{0U}; i < Trunk::SIZE; ++i) {
        trunk->block_map[i] = new BlockInstance *[Trunk::SIZE];
        for (uint32_t j{0U}; j < Trunk::SIZE; ++j) {
            trunk->block_map[i][j] = new BlockInstance[Trunk::SIZE];
            for (uint32_t k{0U}; k < Trunk::SIZE; ++k) {
...
void TrunkMap::link_trunk_at(int64_t x_index, int64_t y_index) {
    Trunk *trunk{this->origin[x_index] + y_index};
    for (int32_t i{0}; i < Trunk::SIZE; ++i) {
        for (int32_t j{0}; j < Trunk::SIZE; ++j) {
            for (int32_t k{0}; k < Trunk::SIZE; ++k) {
                int64_t x{x_index * Trunk::SIZE + i}, y{y_index * Trunk::SIZE + j};
...
void MainActivity::filter_imvisible_blocks() {
    ...
    std::unordered_set<BlockInstance *> block_displayed;
    // this->camera.viewport_vector_mutex.lock();
    for (double_t viewport_x{-1.0}, viewport_vector[3]; viewport_x <= 1.0; viewport_x += 0.01/*0.02*/) {
        for (double_t viewport_y{-1.0}; viewport_y <= 1.0; viewport_y += 0.01) {
            // this->camera.viewport_vector_mutex.lock();
            this->camera.get_viewport_vector(viewport_x, viewport_y, viewport_vector);
            for (double_t depth{64.0}; depth > 0.0 && this->camera.v_get_pixel_coord3llv(viewport_vector, depth, coord); --depth) {
                BlockInstance *block_inst{this->trunk_map.get_block_inst(coord[0], coord[1], coord[2])};
                if (block_displayed.find(block_inst) == block_displayed.end()) {
                    temp_visible_block_list.emplace_back(block_inst);
                    block_displayed.insert(block_inst);
                }
            }
            // this->camera.viewport_vector_mutex.unlock();
        }
    }
    // this->camera.viewport_vector_mutex.unlock();
    this->visible_block_list_mutex.lock();
    this->visible_block_list.swap(temp_visible_block_list);
    // this->visible_block_list.clear();
    // this->visible_block_list = temp_visible_block_list;
    // TCO << this->visible_block_list.size() << '\t' << temp_visible_block_list.size() << '\n';
    this->visible_block_list_mutex.unlock();
}
void MainActivity::world_display() {
    // int64_t coord[3];
    // for (double_t viewport_x{-1.0}; viewport_x <= 1.0; viewport_x += 0.05/*0.02*/) {
    //     for (double_t viewport_y{-1.0}; viewport_y <= 1.0; viewport_y += 0.05) {
    //         this->camera.set_viewport_vector(viewport_x, viewport_y);
    //         for (double_t depth{64.0}; depth > 0.0 && this->camera.get_pixel_coord3llv(depth, coord); --depth) {
...
// TCO<<event->type<<'\n';
// __TEST__::flag=event->block_inst->block->block_type==BT_RED_STONE_BLOCK;
    // std::cout << event->type << '\t' << Sseu::BlockSource::block_name[event->block_inst->block] << '\n';
    if (event->block_inst != &Trunk::null_block_inst) {
        switch (event->type) {
            case ET_PLACE: {
                // BlockInstancek *neighbour;
                ...
                //     // this->refresh_block(neighbour);
                //     event->block_inst->linked_block_list[0] = neighbour;
                // }
                // if ((neighbour = this->trunk_map.get_block_inst(event->block_inst->coord[0], event->block_inst->coord[1], event->block_inst->coord[2] + 1)) != &Trunk::null_block_inst) {
                //     // neighbour->not_refreshed = true;
                //     // this->refresh_block(neighbour);
                //     event->block_inst->linked_block_list[1] = neighbour;
                // }
                ...
                //     // neighbour->not_refreshed = true;
                //     // this->refresh_block(neighbour);
                //     event->block_inst->linked_block_list[2] = neighbour;
                // }
                // if ((neighbour = this->trunk_map.get_block_inst(event->block_inst->coord[0], event->block_inst->coord[1] + 1, event->block_inst->coord[2])) != &Trunk::null_block_inst) {
                //     // neighbour->not_refreshed = true;
                //     // this->refresh_block(neighbour);
                //     event->block_inst->linked_block_list[3] = neighbour;
                // }
                ...
                //     // neighbour->not_refreshed = true;
                //     // this->refresh_block(neighbour);
                //     event->block_inst->linked_block_list[4] = neighbour;
                // }
                // if ((neighbour = this->trunk_map.get_block_inst(event->block_inst->coord[0] + 1, event->block_inst->coord[1], event->block_inst->coord[2])) != &Trunk::null_block_inst) {
                //     // neighbour->not_refreshed = true;
                //     // this->refresh_block(neighbour);
                //     event->block_inst->linked_block_list[5] = neighbour;
                // }
                // // this->refresh_bloc(event->block_inst);
                switch (event->block_inst->block->block_type) {
                    case BT_RED_STONE_BLOCK: {
                        ...
                        for (BlockInstance **linked_block{event->block_inst->linked_block_list}; linked_block - event->block_inst->linked_block_list < 6; ++linked_block) {
                            // if (*linked_block != nullptr && (*linked_block)->not_refreshed && (*linked_block)->block->block_type == BT_RED_STONE_POWDER) {
...
// __TEST__::info(*linked_block);
                ...
                this->refresh_block(event->block_inst);
                                                                        ...
                                                                        // std::cout<<'\n';
                                                                        // for(uint32_t i{1U};i<32U;++i){
                                                                        //     std::cout<<this->trunk_map.get_block_inst(i,3LL,1LL)->rs_signal_strength<<' ';
                                                                        // }
                                                                        // std::cout<<'\n';
                break;
            }
            case ET_BREAK: {
                ...
                switch (broken_block->block_type) {
                    case BT_RED_STONE_BLOCK: {
                        ...
                        // event->block_inst->rs_signal_strength = 0;
                        this->deactivate_rs_circuit(event->block_inst, &rs_sources);
// TCO<<rs_sources.size()<<'\n';
                        for (BlockInstance *rs_source : rs_sources) {
// TCO<<rs_source->coord[0]<<rs_source->coord[1]<<rs_source->coord[2]<<Sseu::BlockSource::block_name[rs_source->block]<<'\n';
...
void MainActivity::refresh_block(BlockInstance *block_inst) {
    ...
    switch (block_inst->block->block_type) {
        // case BT_RED_STONE_BLOCK: {
        //     block_inst->rs_signal_strength = 4;
        //     for (BlockInstance **linked_block{block_inst->linked_block_list}; linked_block - block_inst->linked_block_list < 6; ++linked_block) {
        //         // if (*linked_block != nullptr && (*linked_block)->not_refreshed && (*linked_block)->block->block_type == BT_RED_STONE_POWDER) {
        //         if (*linked_block != nullptr && (*linked_block)->block->block_type == BT_RED_STONE_POWDER) {
        //             this->activate_rs_circuit(*linked_block);
        //         }
        ...
        // }
        case BT_RED_STONE_POWDER: {
// TCO<<block_inst->coord[0]<<'\t'<<block_inst->rs_signal_strength<<'\n';
            // block_inst->rs_signal_strength = 0;
            ...
            for (BlockInstance **linked_block{block_inst->linked_block_list + 2}; linked_block - block_inst->linked_block_list < 6; ++linked_block) {
                if (*linked_block != nullptr) {
                    ...
                    // if ((*linked_block)->rs_signal_strength > block_inst->rs_signal_strength + 1) {
                    //     block_inst->rs_signal_strength = (*linked_block)->rs_signal_strength - 1;
                    // }
                }
                mask >>= 1;
            }
...
// TCO<<block_inst->coord[0]<<'\t'<<block_inst->rs_signal_strength<<'\n';
//             // if (block_inst->rs_signal_strength != 0) {
//             for (BlockInstance **linked_block{block_inst->linked_block_list + 2}; linked_block - block_inst->linked_block_list < 6; ++linked_block) {
//                 if (*linked_block != nullptr && (*linked_block)->not_refreshed && (*linked_block)->block->block_type == BT_RED_STONE_POWDER) {
// // std::cout<<(*linked_block)->coord[0]<<'\t'<<(*linked_block)->rs_signal_strength<<'\n';
// // std::this_thread::sleep_for(std::chrono::milliseconds(250));
//                     this->refresh_block(*linked_block);
//                 }
//             }
//             if (block_inst->linked_block_list != nullptr) {
//                 block_inst->linked_block_list[0]->rs_signal_strength = 1;
//                 this->refresh_block(block_inst->linked_block_list[0]);
//             }
//             // }
// TCO<<block_inst->coord[0]<<'\t'<<block_inst->rs_signal_strength<<'\n';
            break;
...
void MainActivity::activate_rs_circuit(BlockInstance *start) {
    for (BlockInstance **linked_block{start->linked_block_list}; linked_block - start->linked_block_list < 6; ++linked_block) {
        if (*linked_block != nullptr && (*linked_block)->rs_signal_strength > start->rs_signal_strength + 1) {
            start->rs_signal_strength = (*linked_block)->rs_signal_strength - 1;
        }
// if(*linked_block!=nullptr){__TEST__::info(*linked_block);}
    }
// __TEST__::info(start);
    this->refresh_block(start);
    for (BlockInstance **linked_block{start->linked_block_list}; linked_block - start->linked_block_list < 6; ++linked_block) {
        if (*linked_block != nullptr && (*linked_block)->block->block_type == BT_RED_STONE_POWDER && (*linked_block)->rs_signal_strength < start->rs_signal_strength - 1) {
            this->activate_rs_circuit(*linked_block);
        }
    }
}
void MainActivity::deactivate_rs_circuit(BlockInstance *start, std::vector<BlockInstance *> *rs_sources) {
// std::cout<<start->coord[0]<<'\t'<<start->rs_signal_strength<<'\n';
    start->rs_signal_strength = 0;
    this->refresh_block(start);
    for (BlockInstance **linked_block{start->linked_block_list}; linked_block - start->linked_block_list < 6; ++linked_block) {
                                                                if(*linked_block==nullptr){continue;}
        if ((*linked_block)->block->block_type == BT_RED_STONE_BLOCK) {
            rs_sources->emplace_back(start);
        }
        if (*linked_block != nullptr && (*linked_block)->block->block_type == BT_RED_STONE_POWDER && (*linked_block)->rs_signal_strength != 0) {
